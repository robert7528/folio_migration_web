...
def write_holdings_tsv(records_data, output_file):
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        ...
        # Deduplicate by bib_id + location + material_type + call_number
        seen = {}
        for data in records_data:
            key = (data['bib_id'], data['location'], data['material_type'], data['full_call_number'])
            if key not in seen:
                holdings_id = generate_holdings_id(data['bib_id'], data['location'], data['material_type'], data['full_call_number'])
