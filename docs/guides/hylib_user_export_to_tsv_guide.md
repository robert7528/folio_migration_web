...
EBSCO 託管的 FOLIO 走 Kong gateway，**`GET /custom-fields` 讀取 API 沒對外開**（回 404 `no Route matched`，帶不帶 `x-okapi-module-id` 都一樣；同一 token 打 `/groups` 是 200，證明 token/gateway 正常，純粹這條路徑沒開）。所以**別靠 API 查 refId**，改用兩個更直接的辦法：

1. **看 post 的錯誤訊息**（最準）：
   - `Custom fields do not exist: [licenseid]` → 該 refId 的欄位不存在
   - `Custom field's options do not exist: [refId = grade, options: [0]]` → 欄位存在但下拉沒這選項
   - 某欄沒報錯 → 它存在且 refId 正確
2. **小批 test post 當試紙**：取 header + 前 5 筆做迷你 TSV，把 `migration_config.json` 的 `userFile.file_name` 暫時指過去，transform → post：
